...
(defun Find_Simple_File_Name (fname)
  ;;
  ;; TODO newer versions of elisp already have a function that does this. 
  ;; 
  (let (index)
    ...
    ;;
    ;; Now look backwards though the fname looking for the last UNIX
    ;; style pathname separator
    ;;
    (while (and (>= index 0) 
                (not 
                 (equal "/" 
                        (substring fname index (+ index 1)))))
      (setq index (- index 1)))
    ;; index now points to the last separator in fname

    ;; set fname to be only the characters after the last separator
    (setq fname (substring fname (+ 1 index)))
    

    ;; 
    ;; Do that whole process for MessyDOG style separators
    ;;
    (setq index (- (length fname) 1))
    (while (and (>= index 0) 
                (not 
