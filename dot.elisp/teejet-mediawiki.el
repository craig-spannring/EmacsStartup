...
(defcustom mediawiki-site-default "TeeJetWiki"
...
(cond
 ...
 (t
  (defcustom mediawiki-site-alist '(("TeeJetWiki"
                                     "http://bznwiki.spray.com/MediaWiki/mediawiki-current/"
                                     ""
                                     ""
                                     "Main Page"))
    "A list of MediaWiki websites."
...
(defun mediawiki-api-call (sitename action args)
  (let* ((the-url  (mediawiki-make-api-url sitename))
         (arg1     (delq nil
                         (append args (list (cons "format" "xml")
                                            (cons "action" action)))))
         (arg2      (string= action "upload"))
         ;; (garbage (message "posting %s %s %s" the-url arg1 arg2))
         (raw (url-http-post the-url arg1 arg2))
         ;;(garbage2 (message "raw was %s" raw))
         (result (assoc 'api
                        (with-temp-buffer
                              (insert raw)
