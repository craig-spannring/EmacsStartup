...
;;   E      Show changes between working copy and other revision 
;;            using ediff. 
;;   c      Commit files
;;   a      Add files
;;   r      Remove files
...
(defun svn-status-set-default-mode-map ()
  ...
  (define-key svn-status-mode-map "E" 'dsvn-addons-status-ediff)
		  ...
		  (svn-format-help-column
		   '((svn-find-file "visit file")
		     ...
		     (svn-diff-file "show file diff")
                     (dsvn-addons-status-ediff "show file ediff")
...
;;; Local add-ons to dsvn

(eval-when-compile (require 'ediff-util))

(defun dsvn-addons-status-ediff (arg)
  "Run ediff on working file against a different revision. 

When ARG is non-nil then it will prompt for the version to
compare against.
  "
  (interactive "P")

  (let* ((fname            (or (svn-getprop (point) 'file)
                               (svn-getprop (point) 'dir)))
         (basename         (file-name-nondirectory fname))
         (abspath          (expand-file-name fname))
         (dirname          (file-name-directory abspath))
         (rev              (if arg 
                               (read-string "Get files for version: " "PREV")
                             "BASE"))
         (other-rev-fname  (concat (file-name-as-directory dirname) 
                                   basename 
                                   (format ".~%s~" rev)))
         (other-rev-buf    (find-file-noselect other-rev-fname))
         (work-file-buf    (find-file-noselect fname))
         (return-to-buffer (current-buffer))
         ;; Our ediff quit hook needs some paramters, but the hook
         ;; mechanism in ediff-buffers only allows for niladic
         ;; functions.
         ;; 
         ;; exit-ediff-hook is a lambda expression bound with 
         ;; parameters we need for our real hook.  ediff-buffers 
         ;; will call the (niladic) lambda expression and 
         ;; that lambda will in turn call our real hook
         (self-lambda (cons nil nil))
         (exit-ediff-hook  `(lambda ()
                              (dsvn-addons-exit-ediff-hook 
                               ',work-file-buf
                               ',other-rev-buf
                               ',return-to-buffer
                               ',self-lambda))))
    ...
    (setcar self-lambda exit-ediff-hook)
    
    ;; 
    ;; Get the specified version of the file
    ;; 
    (set-buffer other-rev-buf)
    (setq buffer-read-only nil)
    (erase-buffer)
    (message "Getting revision %s for %s" rev basename)
    (cond ((= 0 (call-process "svn" nil other-rev-buf nil 
                              "cat" "-r" rev abspath)))
          (t    (error "Could not get revision %s for %s"
                       rev fname)))
    (set-buffer-modified-p nil)
    (setq buffer-read-only t)

    ;;
    ;; Okay, everything is setup.  We can run ediff now.
    ;; 
    (ediff-buffers other-rev-buf work-file-buf 
                   '((lambda () 
                       (add-hook 'ediff-after-quit-hook-internal
                                 exit-ediff-hook))))))


(defun dsvn-addons-exit-ediff-hook (work-file-buffer other-rev-buffer return-to-buffer self-lambda)
  "cleanup temporary buffers after running dsvn-addons-status-ediff"

  ;; 
  ;; We want to go back to (more or less) the same window
  ;; configuration that we had before running ediff.  Basically 
  ;; we will 
  ;;   1) close the window for other-rev-buffer unless 
  ;;      (= 1 (count-windows) 
  ;;   2) Delete the buffer for other-rev-buffer
  ;;   3) Delete the file for other-rev-buffer (if it wasn't
  ;;      modified).  
  ;;   4) close the window for work-file-buffer, if it isn't 
  ;;      the only window in the frame 
  ;;   5) Switch current buffer to return-to-buffer
  ;; 
  (let* ((window-for-other     (get-buffer-window other-rev-buffer))
         (file-for-other       (buffer-file-name other-rev-buffer))
         (window-for-work-file (get-buffer-window work-file-buffer))
         (file-for-work-file   (buffer-file-name work-file-buffer)))
         
    ;; handle other-rev-buffer
    (when window-for-other
      (switch-to-buffer other-rev-buffer)
      (when (> (count-windows) 1) (delete-window window-for-other)))
    (when (not (buffer-modified-p other-rev-buffer))
        (kill-buffer other-rev-buffer)
        (when (file-exists-p file-for-other) (delete-file file-for-other)))

    ;; Now take care of work-file-buffer
    (when window-for-work-file 
      (switch-to-buffer work-file-buffer)
      (when (> (count-windows) 1) (delete-window window-for-work-file)))

    ;; Return the user to the *svn* buffer and window
    (when (and return-to-buffer
               (buffer-live-p return-to-buffer)
               (not (get-buffer-window return-to-buffer t))) ; is it on display in other frame?
      (ignore-errors (switch-to-buffer return-to-buffer))))
  
  ;; (message "should unregister %s" (car self-lambda))
  (remove-hook 'ediff-after-quit-hook-internal (car self-lambda))
  )
